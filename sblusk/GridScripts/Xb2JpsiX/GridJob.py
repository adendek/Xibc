...
inputdir = '/afs/cern.ch/user/s/sblusk/Xibc/Xb2JpsiX/'
backend=Dirac()
#backend=Local()
...
def getMCDataSet(pol):
    ...
    DecID = "16276061"   # Xibc -> J/psi Xi_c0   sim09a, Digi13, Reco14c
    #DecID = "16276060"   # Xibc -> J/psi Lambda_c+ K-  sim09a, Digi13, Reco14c

    #DecID = "12145090"   # B --> Jpsi K pi pi  sim08i, Digi13, Reco14c - need to restrip
    #DecID = "15144001"   # B --> Jpsi p K   sim08i, Digi13, Reco14c - need to restrip    
    ...
    if DecID == "16265034":
        ...
        vStrip = "20"
        vReco = "Reco14a"
    elif DecID == "16275062" or DecID == "16275061" or DecID == "16276060" or DecID == "16276061":
        sim = "Sim09a"
        vStrip = "21"
        vReco = "Reco14c"
    elif DecID == "12145090":
    ...
    dataDown = "/MC/2012/Beam4000GeV-2012-MagDown-Nu2.5-Pythia8/"+sim+"/Digi13/Trig0x409f0045/"+vReco+"/Stripping"+vStrip+"NoPrescalingFlagged/"
    dataUp = "/MC/2012/Beam4000GeV-2012-MagUp-Nu2.5-Pythia8/"+sim+"/Digi13/Trig0x409f0045/"+vReco+"/Stripping"+vStrip+"NoPrescalingFlagged/"

    if DecID == "16275062" or DecID == "16275061" or DecID == "16276060" or DecID == "16276061":
...
jobname = jobName
if simulation:
    data,jobname,restrip = getMCDataSet(pol)
else:
    data,jobname,restrip = getDataSet(year,pol)

InputData = BKQuery( path = data, dqflag=['ALL']).getDataset()

reStripFile = "noRestrip.py"
if restrip:
    reStripFile = "reStrip.py"
...
if simulation:
    if data.find("MagDown"):
        j.application.optsfile = [File(inputdir+reStripFile), File(inputdir+'isSimulationDown.py'), File(inputdir+'Data2012.py'), File(inputdir+'DataXibc.py')]
    else:
        j.application.optsfile = [File(inputdir+reStripFile), File(inputdir+'isSimulationUp.py'), File(inputdir+'Data2012.py'), File(inputdir+'DataXibc.py')]        
else:
    if year == "2011":
        j.application.optsfile = [File(inputdir+reStripFile), File(inputdir+'isNotSimulationDown.py'), File(inputdir+'Data2011.py'), File(inputdir+'DataXibc.py')]
    else:
        j.application.optsfile = [File(inputdir+reStripFile), File(inputdir+'isNotSimulationDown.py'), File(inputdir+'Data2012.py'), File(inputdir+'DataXibc.py')]
