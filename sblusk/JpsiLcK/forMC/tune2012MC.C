void tune2012MC()
{
  ...
  TString outdir = "/data1/sblusk/Xibc/JpsiX/mc/JpsiLcK/";
  
  TString infile, outfile,treeName;
  TString topdir = outdir;
  infile = topdir+"JpsiLcK_mc.root";
  outfile = outdir+"JpsiLcK_mc.tuned.root";
  ...
  double K_ETA, Pi_ETA, Pr_ETA, Kaon_ETA;
  double K_PZ, Pi_PZ, Pr_PZ, Kaon_PZ;
  double K_PT, Pi_PT, Pr_PT, Kaon_PT;
  ...
  oldtree->SetBranchAddress("Kaon_PT", &Kaon_PT);  
  oldtree->SetBranchAddress("Kaon_PZ", &Kaon_PZ);  
  oldtree->SetBranchAddress("Kaon_IPCHI2_OWNPV", &Kaon_IPCHI2_OWNPV);
  ...
  newtree->Branch("Kaon_ETA", &Kaon_ETA);
  ...
  cout << "Looping over " << nentries << " entries" << endl;
   ...
   for (Long64_t i=0;i<nentries; i++) {
      ...
      Pr_IPCHI2_OWNPV = v;
      voff = 0.15*(1.0+0.05*gRandom->Gaus());
      v = exp(log(Kaon_IPCHI2_OWNPV)+voff);
      Kaon_IPCHI2_OWNPV = v;
      ...
      Kaon_ETA = -1.0*log(tan(0.5*Kaon_PT/Kaon_PZ));
